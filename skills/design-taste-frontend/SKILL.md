...
## Appendix D: Design Search Database (Merged from ui-ux-pro-max)

This skill includes a Python-based design search tool at `scripts/search.py` that queries a curated database of 67 styles, 96 color palettes, 57 font pairings, 99 UX guidelines, and 25 chart types across 13 technology stacks.
...
# Persist design system for session continuity
...
`html-tailwind` (default), `react`, `nextjs`, `vue`, `svelte`, `swiftui`, `react-native`, `flutter`, `shadcn`, `jetpack-compose`, `astro`, `nuxtjs`, `nuxt-ui`

### When to Use

- After inferring the design direction from the brief (Section 0)
- When you need color palette or typography recommendations grounded in data
- When you need stack-specific implementation guidelines
- Before creating the master design system file

### Data Files

All CSV data files live in `data/` — refresh or extend them independently of the SKILL.md.
